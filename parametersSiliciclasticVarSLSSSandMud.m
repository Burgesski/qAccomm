function params = parametersSiliciclasticVarSLSSSandMud(modelName, plotFlag)

    rng(56); % 42 is a good choice, of course

    params = struct; % Create params structure array, primarily to make sure it is empty
    params.siliciclasticModel = 1;
    params.carbonateModel = 0;
    ...
    % params.subsidRateVect = zeros(1,151);

    % Sediment supply parameters
    params.sedimentSupply = zeros(1,params.totalChrons);
    params.sedimentSupply(1) = 5.0E6; % External sediment supply as an area, m2 per My, because 2D model. Enough to fill 100km wide 100m max depth marine accommodation "wedge"
    sedSupplyIncrementAmplitude = 5.0E5; % 0 for simplest model, or 10.0E5 for interesting sed supply forcing;
    for t = 2:params.totalChrons
        params.sedimentSupply(t)  = params.sedimentSupply(t-1) + ((rand * sedSupplyIncrementAmplitude) - (sedSupplyIncrementAmplitude/2));
    end
    params.sedimentSupply = max(params.sedimentSupply, 0); % Replace any negative values with zeros
    params.supplySandProportion = 0.3; % Proportion of total supply volume that is sand
    fprintf("Sediment supply from %2.1f to %2.1f m2My", min(params.sedimentSupply), max(params.sedimentSupply))

    % Single element in the depositional model - rate of decay of deposition rate, per model grid cell
    params.depositionalProportion = 0.5;
    ...
    eustasyAmplitude = 10; % zero to peak oscillation size, in m 10m for SL forced, zero for unforced
    ...
    asymmSeaLevel = [0, asymmSeaLevel(~isnan(asymmSeaLevel))]; % Recalculate the sea level curve, make sure starts with a zero, removing all the NaN values set in the rising limb sections
    % Calculate a random walk curve element
    randomWalkCurve = zeros(1,params.totalChrons);
    eustaticIncrementAmplitude = 1; % Zero for simplest model, 1 for Sl-forced model
    for t = 2:params.totalChrons
    ...
    params.maxDeposRate = 5000.0; % in m per My, 1000m per My is reasonable, based on time-dependent rates from Sadler (1981) - but check unit conversion in code!
    params.terrestrialSandDeposP = 0.01; %
    params.terrestrialMudDeposP = 0.01; % 
    params.marineSandDeposP = 0.1; % Increasing probabilites increases steepness of the marine profile?? i think - need to test 0.05
    params.marineMudDeposP = 0.2; % 0.05 or 0.1
    params.maxErosionPGradientTerrestrial = 0.002; % dimensionless gradient, above which erosion probability is 1; 0.0175 is 1 degree
    params.maxErosionPGradientMarine = 0.035; % dimensionless gradient, above which erosion probability is 1; 0.0175 is 1 degree so 0.035 is realistic
    params.maxErosionRateTerrestrial = 1.0; % in m per My
    params.maxErosionRateMarine = 0.001; % in m per My

    params.colourMap = [0, 0.596, 0.984, 0.596; ... % Water depth less than 0m, so fluvial, pale green       
                        -20, 236/255. 204/255, 162/255; ... % Water depth 20m to 0m so shoreface, sand colour
                        -1000, 0.7, 0.7, 0.7]; % Everything else below 20m down to 100m max, offshore marine, light grey
    ...
    if plotFlag
        ...
        nexttile(2,[4,1])
        plot(params.sedimentSupply, EMTVector, "Color",[0.7,0,0]);
        grid on
        xlabel("Sediment supply (m2My-1)")
        ylabel("Elapsed model time (My)")
        % titleStr = sprintf("Rand seed %d", randSeedValue);
        % title(titleStr)
        drawnow
    end
end
